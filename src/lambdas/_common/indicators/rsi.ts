...
import { wildersSmoothing2 } from "./wildersSmoothing";
...
export function rsi(data: ArrayCandle[], period: number) {
	...
	let gainAvgs = wildersSmoothing2( gains, period );
	let loseAvgs = wildersSmoothing2( loses, period );

	return [
		...numberArray(0, period), 
		...gainAvgs.map( (gainAvg, i) => (
			100-(100/(1+(gainAvg/loseAvgs[i])))
		))
	]
}
