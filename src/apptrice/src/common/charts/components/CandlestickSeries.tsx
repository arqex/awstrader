...
export class CandlestickSeries extends React.Component<CandlestickSeriesProps> {
    ...
    private readonly getCandleData = (
        ...
        plotData: any[],
				...
				// For performance boost, not using yAccessor (we know is a ChartData)
        ...
        return plotData
            ...
            .map((d) => {
                const {1:open, 2:close, 3:high, 4:low} = d;

                const xValue = xAccessor(d);
                const x = Math.round(xScale(xValue));
                const y = Math.round(yScale(Math.max(open, close)));
                const height = Math.max(1, Math.round(Math.abs(yScale(open) - yScale(close))));
                ...
                return {
                    ...
                    wick: {
                        stroke: wickStroke(d),
                        x,
                        y1: Math.round(yScale(high)),
                        y2: y,
                        y3: y + height,
                        y4: Math.round(yScale(low)),
                    },
                    height,
                    width: offset * 2,
                    fill: fill(d),
